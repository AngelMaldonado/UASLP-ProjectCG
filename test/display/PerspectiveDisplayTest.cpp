...
void TestFramework::run() {
    TransformationsHandler::rotateObject(cube, cube.getOrigin(), 0.5, 0, 0);
}
...
void SimulationFramework::keyboard(unsigned char key, int x, int y) {
    ...
    {
        case 120:
            trajectoryHandler.setHermiteValues(LEFT_CURVE_X, CURVE_Y, 0, RIGHT_CURVE_X, CURVE_Y, 0, LEFT_CURVE_X, ++yLEFT, 0, RIGHT_CURVE_X, yRIGHT, 0, 0.001, 1, 1, 1, 1, GL_POINTS);
        break;
        case 122:
            trajectoryHandler.setHermiteValues(LEFT_CURVE_X, CURVE_Y, 0, RIGHT_CURVE_X, CURVE_Y, 0, LEFT_CURVE_X, --yLEFT, 0, RIGHT_CURVE_X, yRIGHT, 0, 0.001, 1, 1, 1, 1, GL_POINTS);
        break;
...
void givenModel_whenPerspectiveViewAndCertainDisplayFunction_thenModelDisplays(int argc, char* argv[]) {
    ...
    simFramework->ship = Object(reader1.readFile("objs/Starship.obj", 'n'));
    ...
    simFramework->ship.setBrush(119, 140, 130, 1, GL_POLYGON);
    ...
    simFramework->base.setBrush(1, 2, 130, 1, GL_POLYGON);

    // FLOOR
    TransformationsHandler::scaleObject(simFramework->worldFloor, 50);
    simFramework->worldFloor.setBrush(0, 100, 0, 1, GL_POLYGON);
...
void visibleTest(int argc, char* argv[]) {
    ...
    TestFramework *simFramework = new TestFramework();
    GLInterface *glInterface = new GLInterface(1024, 576, 0, 0, "Perspective Display - Test");
    simFramework->cube = Object(reader1.readFile("objs/test/cube.obj", 'n'));
    
    // SHIP
    TransformationsHandler::scaleObject(simFramework->cube, 20);
    simFramework->cube.setOrigin(0, 0, 0);
    simFramework->cube.setBrush(119, 140, 130, 1, GL_LINE_LOOP);
